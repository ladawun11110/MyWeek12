...
<head>
    ...
    <title>แก้ไขบทความ - Admin Dashboard</title>
    <!-- Tailwind CSS -->
...
<body class="bg-slate-50 min-h-screen text-slate-800">
    ...
    <div class="container mx-auto px-4 py-12 max-w-2xl">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">แก้ไขบทความ</h1>
                <p class="text-slate-500 mt-1">อัปเดตหรือปรับเปลี่ยนข้อมูลรายละเอียดของบทความนี้</p>
            </div>
            <a href="{{ route('blog2') }}" 
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden p-8">
            <form action="{{ route('update', $blogs->id) }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea id="content" name="content" rows="8" required
                        ...
                        placeholder="เขียนคำบรรยายหรือรายละเอียดของบทความที่นี่...">{{ $blogs->content }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <select id="status" name="status"
                            class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-lg focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition duration-150 ease-in-out text-slate-950 font-medium">
                            <option value="1" {{ $blogs->status == 1 ? 'selected' : '' }}>เผยแพร่แล้ว (Published)</option>
                            <option value="0" {{ $blogs->status == 0 ? 'selected' : '' }}>ฉบับร่าง (Draft)</option>
                        </select>
                    </div>
                </div>
